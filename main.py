...
from tts_piper import tts_piper_to_wav
...
def run_pipeline():
    ...
    # Flush camera buffer - baca dan buang frame lama untuk mendapatkan frame terbaru
    print("[BUFFER] Membersihkan buffer kamera untuk frame terbaru...")
    for _ in range(5):  # Baca 5 frame terakhir untuk memastikan buffer kosong
        cap.read()
    
    # Capture frame terbaru
    ok, frame = cap.read()
    if not ok:
    ...
    print(f"[1/5] Frame disimpan: {img_path}")
    
    # Build prompt (no segmentation)
    t0 = time.time()
    prompt = build_prompt()
    ...
    print(f"[2/5] Encoding & prompt selesai ({latency['encode_image']:.3f}s)")
    
    # Moondream inference (no segmentation)
    t0 = time.time()
    en_raw = query_moondream(img_path, prompt)
    latency["moondream_inference"] = time.time() - t0
    print(f"[3/5] Moondream: {en_raw[:50]}... ({latency['moondream_inference']:.3f}s)")
    ...
    print(f"[4/5] Terjemahan: {id_tts[:50]}... ({latency['translation']:.3f}s)")
    
    # TTS (generate only, no play during latency measurement)
    t0 = time.time()
    wav_path = tts_piper_to_wav(id_tts)
    latency["tts"] = time.time() - t0
    print(f"[5/5] TTS selesai: {wav_path} ({latency['tts']:.3f}s)")
    ...
    print(f"Pipeline selesai: {latency['total_pipeline']:.3f}s (wall: {wall_time:.3f}s)")
    
    # Play audio AFTER latency calculation
    if wav_path and os.path.exists(wav_path):
        print(f"[Playing audio]: {wav_path}")
        try:
            import subprocess
            subprocess.Popen(["aplay", wav_path])  # Non-blocking playback on Jetson
        except Exception as e:
            print(f"[Warning] Cannot play audio: {e}")
